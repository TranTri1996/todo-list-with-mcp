...
server.resource('todos', 'todos://', async () => {
...
server.resource('todo', new ResourceTemplate('todo://{id}', { list: undefined }), async (uri, variables) => {
...
server.tool('create_todo', { title: z.string().min(1, "Title cannot be empty") }, async ({ title }) => {
    try {
        ...
        return {
            content: [{ type: 'text', text: `Error creating todo: ${error.message}` }],
            isError: true,
        };
    }
});
...
server.tool('update_todo', {
    ...
    try {
        ...
        return {
            content: [{ type: 'text', text: `Error updating todo: ${error.message}` }],
            isError: true,
        };
    }
});
// Tool to delete a todo
server.tool('delete_todo', { id: z.string().min(1, "ID cannot be empty") }, async ({ id }) => {
    try {
...
server.tool('list_todos', {}, async () => {
    try {
        ...
        return {
            content: [{ type: 'text', text: `Error fetching todos: ${error.message}` }],
            isError: true,
        };
    }
});
